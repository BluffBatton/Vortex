...
const fuels = [
...

	{
		...
		description:
			'Gas Premium – high-calorific methane gas (CH₄) compliant with Euro5 standards, with minimal impurities (≤0.1%).',
		...
		<>
			...
			{fuels.map(fuel => (
				<div
					...
					onClick={e => {
						if (e.target.classList.contains('fuel-modal')) {
							closeModal()
						}
					}}
				>
					<div className='fuel-modal__content'>
						...
						<div className='fuel-modal__body'>
							{fuel.description && <p>{fuel.description}</p>}
							<h3>Technology</h3>
							<p>{fuel.technology}</p>
						</div>
					</div>
